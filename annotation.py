...
def drawimage(left_image, right_image, points, cur_point_idx):
    ...
    if len(points) == 0:
        ...
        dif = left_expand - right_expand

        color = np.zeros((11, 11, 3), dtype=np.uint8)
        color[:, :, 0] = 255
        x1 = expand_window_size//2 - 5
        x2 = expand_window_size//2 + 6
        dif = np.abs(left_expand[x1:x2, x1:x2, :] - right_expand[x1:x2, x1:x2, :]).mean()
...
if __name__ == '__main__':
    imagelist = os.listdir('./sceneflow-sample/sgm/')
    ...
    if os.path.isfile('./sceneflow-sample/anno/' + imagelist[cur_image_idx].split('.')[0] + '.npy'):
        points = np.load('./sceneflow-sample/anno/' + imagelist[cur_image_idx].split('.')[0] + '.npy')
        points = points.tolist()
    else:
        points = []

    while True:
        left_name = './sceneflow-sample/leftimage/' + imagelist[cur_image_idx]
        right_name = './sceneflow-sample/rightimage/' + imagelist[cur_image_idx]
        output_name = './sceneflow-sample/anno/' + imagelist[cur_image_idx].split('.')[0] + '.npy'
